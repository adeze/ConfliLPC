from simpletransformers.classification import MultiLabelClassificationArgs, ClassificationArgs
from simpletransformers.ner import NERModel, NERArgs
from models_ewc.multi_label_classification_model import MultiLabelClassificationModel_EWC
from models_ewc.classification_model import ClassificationModel_EWC
        ...
        if len(dirs) == 0:
            ...
            for all_task_id in range(task_id + 1):
                ...
                if args.task == "multilabel":
                    ...
                    model = MultiLabelClassificationModel_EWC(model_configs["architecture"], checkpoint_dir, num_labels=args.num_labels, args=model_args, ignore_mismatched_sizes=True)
                    ...
                    model = ClassificationModel_EWC(model_configs["architecture"], checkpoint_dir, num_labels=len(unique_labels), args=model_args, ignore_mismatched_sizes=True)
                    ...
                    # Create a MultiLabelClassificationModel
                    model = ClassificationModel_EWC(model_configs["architecture"], checkpoint_dir, num_labels=args.num_labels, args=model_args, ignore_mismatched_sizes=True)
...
def train_binary(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    unique_labels = np.unique(test_df['labels'])
    model = ClassificationModel_EWC(architecture, pretrained_model, num_labels=len(unique_labels), args=model_args, ignore_mismatched_sizes=True)
...
def train_multilabel(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        ...
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
    model = MultiLabelClassificationModel_EWC(architecture, pretrained_model, num_labels=args.num_labels, args=model_args, ignore_mismatched_sizes=True)
...
def train_multiclass(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        ...
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
    model = ClassificationModel_EWC(architecture, pretrained_model, num_labels=args.num_labels, args=model_args, ignore_mismatched_sizes=True)
