        ...
        if len(dirs) == 0:
            ...
            for all_task_id in range(task_id + 1):
                ...
                if args.task != 'qa':
                    ...
                    train_df, eval_df, test_df, args.num_labels = None, None, None, 2
                ...
                if args.task == "multilabel":
                    ...
                    results = model_finetune_object.QA_metric_compute(predictions=predicted_answers, references=theoretical_answers)
...
def train_binary(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    print(f"Starting train_binary for seed {seed}")
    ...
    unique_labels = np.unique(test_df['labels'])
    model = ClassificationModel(architecture, pretrained_model, num_labels=len(unique_labels), args=model_args,
...
def train_multilabel(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        ...
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
    model = MultiLabelClassificationModel(architecture, pretrained_model, num_labels=args.num_labels, args=model_args, ignore_mismatched_sizes=True)
...
def train_multiclass(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        ...
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
    model = ClassificationModel(architecture, pretrained_model, num_labels=args.num_labels, args=model_args, ignore_mismatched_sizes=True)
...
def train_ner(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        ...
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
    model = NERModel(architecture, pretrained_model, args=model_args, ignore_mismatched_sizes=True)
