...
import os
import json
from bert_qa_finetune_lpc import *
...
def report_per_epoch(args, task_id, test_df, seed, model_configs):
    ...
    for epoch in range(1, args.epochs_per_seed + 1):
        ...
        dirs = [f for f in os.listdir(args.output_dir) if
                f[-len(end_dir):] == end_dir and os.path.isdir(os.path.join(args.output_dir, f))]
        ...
        if len(dirs) == 0:
            ...
            for all_task_id in range(task_id + 1):
                ...
                if args.task != 'qa':
                    ...
                    train_df, eval_df, test_df, args.num_labels = None, None, None, -1
                ...
                if args.task == "multilabel":
                    ...
                    model = MultiLabelClassificationModel(model_configs["architecture"], checkpoint_dir,
                                                          num_labels=args.num_labels, args=model_args,
                    ...
                    result["acc"] = example_based_accuracy(
                        [list(test_df.labels[i]) for i, pred in enumerate(predictions)],
                        [list(pred) for pred in predictions])
                    result["prec"] = example_based_precision(
                        [list(test_df.labels[i]) for i, pred in enumerate(predictions)],
                        [list(pred) for pred in predictions])
                    result["rec"] = example_based_recall(
                        [list(test_df.labels[i]) for i, pred in enumerate(predictions)],
                        [list(pred) for pred in predictions])
                    result["f1"] = example_based_f1([list(test_df.labels[i]) for i, pred in enumerate(predictions)],
                                                    [list(pred) for pred in predictions])
                    ...
                    model = NERModel(model_configs["architecture"], checkpoint_dir, args=model_args,
                                     ignore_mismatched_sizes=True)
                    ...
                    model = ClassificationModel(model_configs["architecture"], checkpoint_dir,
                                                num_labels=len(unique_labels), args=model_args,
                                                ignore_mismatched_sizes=True)
                    ...
                    # Training Arguments
                    orig_model_args = args
                    # Convert args to dictionary if it's a Namespace object
                    if isinstance(orig_model_args, argparse.Namespace):
                        orig_model_args = vars(orig_model_args)
                    orig_model_args['model_local_path'] = checkpoint_dir

                    model_finetune_object = QA_Finetune(**orig_model_args)

                    train_examples, eval_examples = model_finetune_object.load_qa_dataset()
                    ...
                    # Initialize the QA model with SimpleTransformers' QuestionAnsweringModel
                    ...
                    predictions, raw_outputs = qa_model.predict(eval_examples)
                    ...
                    results = model_finetune_object.QA_metric_compute(predictions=predicted_answers,
                                                                      references=theoretical_answers)

                    # Collecting relevant results
                    result = {}

                    result["acc"] = results['exact_match']
                    result["f1"] = results['f1']
...
def train_multi_seed(args, task_id, train_df, eval_df, test_df, model, model_configs):
    ...
    for curr_seed in range(init_seed, init_seed + args.num_of_seeds):
        ...
        if args.task == "multiclass":
            ...
            model, result = train_qa(args, task_id, train_df, eval_df, test_df, curr_seed, model, model_configs)

        # Recording best results for a given seed
        log_filename = os.path.join("./logs/", args.tasks[task_id] + "_best_results.json")
        out_dict = {"data_name": args.tasks[task_id], "model_name": model_configs["model_name"], "seed": curr_seed,
...
def train_binary(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    model = ClassificationModel_LPC(architecture, pretrained_model, num_labels=len(unique_labels),
                                    previous_model=prev_model, args=model_args, ignore_mismatched_sizes=True)
...
def train_multilabel(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        pretrained_model = model_configs["model_path"]
        prev_model = MultiLabelClassificationModel(architecture, pretrained_model, num_labels=args.num_labels,
                                                   args=model_args, ignore_mismatched_sizes=True)
    else:
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
        prev_model = MultiLabelClassificationModel(architecture, pretrained_model, num_labels=args.num_labels,
    ...
    model = MultiLabelClassificationModel_LPC(architecture, pretrained_model, num_labels=args.num_labels,
                                              previous_model=prev_model, args=model_args, ignore_mismatched_sizes=True)
...
def train_multiclass(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    if task_id == 0:
        ...
        prev_model = ClassificationModel(architecture, pretrained_model, num_labels=args.num_labels, args=model_args,
                                         ignore_mismatched_sizes=True)
    model = ClassificationModel_LPC(architecture, pretrained_model, num_labels=args.num_labels,
                                    previous_model=prev_model, args=model_args, ignore_mismatched_sizes=True)
...
def train_ner(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    ...
    model = NERModel_LPC(architecture, pretrained_model, previous_model=prev_model, args=model_args,
                         ignore_mismatched_sizes=True)
...
def train_qa(args, task_id, train_df, eval_df, test_df, seed, model, model_configs):
    model_args = args

    model_args.task = args.task
    model_args.task_id = task_id
    # Convert args to dictionary if it's a Namespace object
    if isinstance(model_args, argparse.Namespace):
        model_args = vars(model_args)
    model_args['task_id'] = task_id
    if task_id == 0:
        pretrained_model = model_configs["model_huggingface_path"]
        model_args['model_huggingface_path'] = pretrained_model
    else:
        model_dir = os.path.join("./outputs/", args.tasks[task_id - 1], "best_model")
        pretrained_model = model_dir
        model_args['model_local_path'] = pretrained_model

    model_finetune_object = QA_Finetune(**model_args)
    results = model_finetune_object.fine_tune_all_seeds()
...
def main():
    ...
    parser.add_argument('--model_huggingface_path', type=str,
                        help='Path to model, if the model is stored on huggingface.')

    parser.add_argument('--dataset_local_path', type=str,
                        help='Path to QA dataset, if the QA dataset is stored locally.')
    parser.add_argument('--dataset_huggingface_path', type=str,
                        help='Path to QA dataset, if the QA dataset is stored on huggingface.')
    parser.add_argument('--dataset_config_name', type=str,
                        help='Some datasets require a config name, usually a version.')

    parser.add_argument('--final_model_name', type=str,
                        help='Name of model after fine-tuning is completed.')
    parser.add_argument('--final_model_output_directory', type=str,
                        help='Output directory for model after fine-tuning is completed. End with \'\\\'')

    # Model parameter arguments
    parser.add_argument('--language', type=str, help='Language that the model and dataset are in.')
    parser.add_argument('--num_epochs', type=int, help='Number of epochs for each GPU.')
    # parser.add_argument('--learning_rate', type=float,
    #                     help='Learning rate for model. Written in scientific notation. Ex: 5e-5')
    parser.add_argument('--num_seeds', type=int,
                        help='Number of seeds for fine-tuning. Best performing seed is saved.')
    parser.add_argument('--batch_size', type=int, help='Size of batch for fine-tuning.')
    parser.add_argument('--seed', type=int, help='Exact seed to use.')

    parser.add_argument('--max_answer_length', default=30, type=int,
                        help='Maximum answer length for a model\'s answer to a question.')
    parser.add_argument('--max_context_length', default=384, type=int,
                        help='Maximum length for a context. Bert\'s limit & suggested length is 384.')
    parser.add_argument('--stride', default=128, type=int,
                        help='Stride length for trimming size of QA dataset contexts.')

    # Path specific arguments
    parser.add_argument('--use_this_gpu', required=False, type=str,
                        help='Allows the use of only 1 gpu. Specify the id of the gpu you would like to use.',
                        default='None')
        ...
        for model_configs in args.models:
            ...
            if task_id == 0:
                if args.task != 'qa':
                    ...
                    model = model_configs['model_huggingface_path']
