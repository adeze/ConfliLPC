...
from models_lpc.LPC import *
from models_lpc.loss_lc import CELCLoss


logger = logging.getLogger(__name__)


class QuestionAnsweringModel_LPC:
    def __init__(
        self, model_type, model_name, args=None, use_cuda=True, cuda_device=-1, previous_model=None, **kwargs
    ):
        """
    ...
    def train(
        ...
        **kwargs,
        ...
        optimizer_grouped_parameters = [
            {
                ...
                "pretrain_params": [p_p for p_n, p_p in self.previous_model.model.named_parameters() if
        ...
        if args.optimizer == "AdamW":
            ...
            optimizer = LPC(optimizer_grouped_parameters, reg_lambda=args.reg_lambda, lr=args.learning_rate,
                            eps=args.adam_epsilon,
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if args.fp16:
                    with amp.autocast():
                        prev_outputs = self.previous_model.model(**inputs)
                        outputs = model(**inputs)
                        # model outputs are always tuple in pytorch-transformers (see doc)
                        prev_start_logits, prev_end_logits = prev_outputs[1], prev_outputs[2]
                        start_logits, end_logits = outputs[1], outputs[2]
                        self.loss_fct = CELCLoss()
                        ...
                        if start_positions is not None and end_positions is not None:
                            ...
                            start_loss = self.loss_fct(start_logits, prev_start_logits, start_positions, args.logits_calibraion_degree)
                            end_loss = self.loss_fct(end_logits, prev_end_logits, end_positions, args.logits_calibraion_degree)
                            loss = (start_loss + end_loss) / 2
                        # loss = (logits.view(-1, args.num_labels), prev_logits.view(-1, args.num_labels),
                        #                 labels.view(-1),
                        #                 args.logits_calibraion_degree)
                else:
                    prev_outputs = self.previous_model.model(**inputs)
                    outputs = model(**inputs)
                    # model outputs are always tuple in pytorch-transformers (see doc)
                    ...
                    if start_positions is not None and end_positions is not None:
                        ...
                        start_loss = self.loss_fct(start_logits, prev_start_logits, start_positions,
                                                   args.logits_calibraion_degree)
                        end_loss = self.loss_fct(end_logits, prev_end_logits, end_positions,
                                                 args.logits_calibraion_degree)
                        loss = (start_loss + end_loss) / 2
                    # loss = (logits.view(-1, args.num_labels), prev_logits.view(-1, args.num_labels),
                    #                 labels.view(-1),
            ...
            if epoch_number % args.update_epoch == 0:
                optimizer_ft = LPC_omega_update(model.reg_params)
                model = compute_omega_grads_norm(args=args, model=model, dataloader=train_dataloader,
    ...
    def _move_model_to_device(self):
        ...
        self.previous_model.model.to(self.device)
