...
class ClassificationModel_LPC:
    def __init__(
        ...
        **kwargs,
        ...
        if self.weight:
            self.loss_fct = CELCLoss(weight=torch.Tensor(self.weight).to(self.device))
        else:
    ...
    def train(
        ...
        **kwargs,
        ...
        for _ in train_iterator:
            model.train()
            self.previous_model.model.eval()
            if epochs_trained > 0:
                epochs_trained -= 1
                continue
            ...
            if epoch_number % args.update_epoch == 0:
                ...
                model = compute_omega_grads_norm(args=args, model=model, dataloader=train_dataloader, optimizer=optimizer_ft)
                model.eval()
    ...
    def load_and_cache_examples(
        ...
        if args.sliding_window or self.args.model_type in ["layoutlm", "layoutlmv2"] or args.optimizer == "LPC":
    ...
    def _calculate_loss(self, model, inputs, loss_fct, num_labels, args):
        ...
        loss = outputs[0]
        if loss_fct:
            prev_logits = prev_outputs[1]
            logits = outputs[1]
            labels = inputs["labels"]

            # loss = loss_fct(logits.view(-1, num_labels), labels.view(-1))
            loss = loss_fct(logits.view(-1, num_labels), prev_logits.view(-1, num_labels), labels.view(-1),
